...
def create_contour_plot(ind):
    ...
    for i, symbol in enumerate(symbols[keys[ind]]):
        coords[i] = symbol.coord

    base_dist, _ = nav.dijkstra(symbols[keys[0]], start, goal, distance, complete = True, time_cost= True)
    dist, prev = nav.dijkstra(symbols[keys[ind]], start, goal, distance, complete = True, time_cost= True)
    vmax = max(base_dist)

    make_contour(coords, dist, ax, 0, vmax, int(15//(vmax/max(dist))))
    plt.show()

def create_contour_mosaic():
    with np.load("navigation/result_data/400_1ms_2.npz", allow_pickle=True) as data:
        inhibit_range= data['inhibit_ranges']
        symbols = data['symbols']
        start = data['start']
