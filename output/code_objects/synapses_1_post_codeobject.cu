...
int getThreadsPerBundle(){
    ...
    int threads_per_bundle = static_cast<int>(synapses_1_post_bundle_size_max);
    ...
    )
{
    ...
    ///// KERNEL_CONSTANTS /////
    const double Apost = - 0.12;
	const int _numN = 1;
	const int _numapost_pre = 7488;
	const int _numapre_pre = 7488;
	const double c_max = 0.052083333333333336;
	const int _numc_pre = 7488;
	const int _numl_speed_pre = 7488;
    ...
    {  // _idx is defined in outer and inner scope (for `scalar_code`)
        if (synapses_1_post.no_or_const_delay_mode)
        {
            ...
            if (timestep >= synapses_1_post.queue->num_delays)
            {
                        ...
                        spiking_neuron_idx++)
                {
                    ...
                    if(spikes_start <= spiking_neuron && spiking_neuron < spikes_stop)
                    {
                        ...
                        for(int j = tid; j < num_synapses; j+=_threads_per_block)
                        {
                            ...
                            double apost_pre = _ptr_array_neurongroup_apost[_presynaptic_idx];
                            const double apre_pre = _ptr_array_neurongroup_apre[_presynaptic_idx];
                            double c_pre = _ptr_array_neurongroup_c[_presynaptic_idx];
                            const double l_speed_pre = _ptr_array_neurongroup_l_speed[_presynaptic_idx];
                            c_pre = _brian_clip(c_pre + (l_speed_pre * apre_pre), 0, c_max);
                            apost_pre += Apost;
                            _ptr_array_neurongroup_apost[_presynaptic_idx] = apost_pre;
                            _ptr_array_neurongroup_c[_presynaptic_idx] = c_pre;
...
num_loops = 1;
num_threads = 1;
if (!synapses_1_multiple_pre_post){
    if (synapses_1_post_scalar_delay)
        ...
        num_threads = synapses_1_post_bundle_size_max;
}
else {
    printf("WARNING: Detected multiple synapses for same (pre, post) neuron "
           "pair in Synapses object ``synapses_1`` and no atomic operations are used. "
           "Falling back to serialised effect application for SynapticPathway "
           "``synapses_1_post``. This will be slow. You can avoid serialisation "
