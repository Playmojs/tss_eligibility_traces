    ...
    bool scalar_delay)
{
    ...
    if (scalar_delay)
    {
        if (tid == 0)
        {
            synapses_1_post.queue->num_blocks = _num_blocks;
            synapses_1_post.queue->num_delays = num_delays;
        }
    }
    else
    {
    ...
    synapses_1_post.no_or_const_delay_mode = scalar_delay;
}
...
void _before_run_synapses_1_post_push_spikes()
{
    ...
    for(int i = 0; i < num_pre_post_blocks; i++)
    {
        ...
        if (!scalar_delay)
        {
            ...
            for (int bundle_idx = 0; bundle_idx < num_unique_elements; bundle_idx++)
            {
                ...
                if (bundle_idx == 0){
                    synapses_1_post_bundle_size_min = num_synapses;
                    synapses_1_post_bundle_size_max = num_synapses;
                }
                else {
                    if (num_synapses > synapses_1_post_bundle_size_max){
                        synapses_1_post_bundle_size_max = num_synapses;
                    }
                    if (num_synapses < synapses_1_post_bundle_size_min){
                        synapses_1_post_bundle_size_min = num_synapses;
                    }
                }
    ...
    if (scalar_delay)
    {
        ...
        COPY_HOST_ARRAY_TO_DEVICE_SYMBOL(d_ptr_num_synapses_by_pre,
                h_num_synapses_by_pre, synapses_1_post_num_synapses_by_pre,
                num_pre_post_blocks, "number of synapses per pre/post block");
        // synapses id
        COPY_HOST_ARRAY_TO_DEVICE_SYMBOL(d_ptr_d_ptr_synapse_ids_by_pre,
                h_ptr_d_ptr_synapse_ids_by_pre, synapses_1_post_synapse_ids_by_pre,
                num_pre_post_blocks,
    ...
    else  // not scalar_delay
    {
        ...
        synapses_1_post_bundle_size_mean = mean_bundle_sizes;
        synapses_1_post_bundle_size_std = getStd(count_bundle_sizes, M2_bundle_sizes);
