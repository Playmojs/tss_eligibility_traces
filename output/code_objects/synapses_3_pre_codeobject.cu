    ...
    )
{
    ...
    const double _lio_1 = 0.01 * Ndendrites2;
                ...
                int post_block_idx = bid % num_parallel_blocks;
                {
                    ...
                    if(spikes_start <= spiking_neuron && spiking_neuron < spikes_stop)
                    {
                        ...
                        for(int j = tid; j < num_synapses; j+=_threads_per_block)
                        {
                            ...
                            //  Abstract code:  y_post += _lio_1
                            const int32_t _postsynaptic_idx = _ptr_array_synapses_3__synaptic_post[_idx];
                            _brian_atomicAdd(&_ptr_array_neurongroup_1_y[_postsynaptic_idx], (double)(_lio_1));
    ...
    if (synapses_3_pre_scalar_delay)
    {
        if (defaultclock.timestep[0] >= synapses_3_pre_delay)
        {
            cudaMemcpy(&num_spiking_neurons,
                    &dev_array_neurongroup_2__spikespace[synapses_3_pre_eventspace_idx][_num__array_neurongroup_2__spikespace - 1],
    ...
    if (num_blocks != 0) {
        for(int bid_offset = 0; bid_offset < num_loops; bid_offset++)
        {
            _run_kernel_synapses_3_pre_codeobject<<<num_blocks, num_threads>>>(
                ...
                num_threads_per_bundle,
                dev_array_neurongroup_2__spikespace[synapses_3_pre_eventspace_idx],
                num_spiking_neurons,
                ///// HOST_PARAMETERS /////
