...
void _before_run_synapses_pre_push_spikes()
{
    ...
    ///// HOST_CONSTANTS ///////////
    const int _numN = 1;
		const int32_t _n_sources = 576;
		const int32_t _n_targets = 7488;
		const int _num_spikespace = 577;
    ...
    if (scalar_delay)
    {
        ...
        if (require_new_eventspaces)
        {
            ...
            std::rotate(
                dev_array_spikegeneratorgroup__spikespace.begin(),
                dev_array_spikegeneratorgroup__spikespace.begin() + current_idx_array_spikegeneratorgroup__spikespace,
                dev_array_spikegeneratorgroup__spikespace.end()
            );
            current_idx_array_spikegeneratorgroup__spikespace = 0;
            // add new eventspaces
            for (int i = num_eventspaces; i < num_queues; i++)
            {
                ...
                CUDA_SAFE_CALL(
                    cudaMemcpy(
                        new_eventspace,
                        _array_spikegeneratorgroup__spikespace,  // defined in objects.cu
                        sizeof(int32_t) * _num__array_spikegeneratorgroup__spikespace,
                        cudaMemcpyHostToDevice
                    )
                );
