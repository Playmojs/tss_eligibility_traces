...
__global__ void
_run_kernel_synapses_1_summed_variable_Igap_post_codeobject(
    int _N,
    int THREADS_PER_BLOCK,
    ///// KERNEL_PARAMETERS /////
    double* _ptr_array_neurongroup_1_Igap,
    ...
    )
{
    ...
    ///// KERNEL_CONSTANTS /////
    const int _numIgap_post = 13;
	const int _numN = 1;
	const int32_t N_post = 13;
	const int _num_Ve_pre_neurongroup_c = 7488;
	const int _num_Ve_pre_neurongroup_v = 7488;
...

    {
        ...
        const double _Ve_pre_neurongroup_c = _ptr_array_neurongroup_c[_presynaptic_idx];
        const double _Ve_pre_neurongroup_v = _ptr_array_neurongroup_v[_presynaptic_idx];
        const double Ve_pre = _brian_tanh(_Ve_pre_neurongroup_v) * _Ve_pre_neurongroup_c;
        const double _synaptic_var = Ve_pre;


int _target_id = _ptr_array_synapses_1__synaptic_post[_idx];
_brian_atomicAdd(&_ptr_array_neurongroup_1_Igap[_target_id], _synaptic_var);
    }
}



void _run_synapses_1_summed_variable_Igap_post_codeobject()
{
    using namespace brian;
    ...
    if (first_run)
    {
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxPotentialBlockSize(&min_num_threads, &num_threads,
                    _run_kernel_synapses_1_summed_variable_Igap_post_codeobject, 0, 0)  // last args: dynamicSMemSize, blockSizeLimit
                );
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    _run_kernel_synapses_1_summed_variable_Igap_post_codeobject, num_threads, needed_shared_memory)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, _run_kernel_synapses_1_summed_variable_Igap_post_codeobject)
                );
        ...
        {
            printf("INFO _run_kernel_synapses_1_summed_variable_Igap_post_codeobject\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
...
CUDA_SAFE_CALL(
        cudaMemset(dev_array_neurongroup_1_Igap + 0,
                   0,
                   _target_size * sizeof(double))
        );

    _run_kernel_synapses_1_summed_variable_Igap_post_codeobject<<<num_blocks, num_threads>>>(
            _N,
            num_threads,
            ///// HOST_PARAMETERS /////
            dev_array_neurongroup_1_Igap,
    ...
    CUDA_CHECK_ERROR("_run_kernel_synapses_1_summed_variable_Igap_post_codeobject");
