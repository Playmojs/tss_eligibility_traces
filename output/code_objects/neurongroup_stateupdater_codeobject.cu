    ...
    )
{
    ...
    ///// KERNEL_CONSTANTS /////
    const int32_t N = 7488;
	const int _numapost = 7488;
	const int _numapre = 7488;
	const double tau_d = 0.01;
	const double taupost = 0.05;
	const double taupre = 0.02;
	const int _numv = 7488;
    ...
    const double _lio_1 = _brian_exp(1.0f*(- dt)/taupost);
    const double _lio_2 = _brian_exp(1.0f*(- dt)/taupre);
    const double _lio_3 = _brian_exp(1.0f*(- dt)/tau_d);


    {
        ///// vector_code /////
                
        double apost = _ptr_array_neurongroup_apost[_idx];
        double apre = _ptr_array_neurongroup_apre[_idx];
        double v = _ptr_array_neurongroup_v[_idx];
        const double _apost = _lio_1 * apost;
        const double _apre = _lio_2 * apre;
        const double _v = _lio_3 * v;
        apost = _apost;
        apre = _apre;
        v = _v;
        _ptr_array_neurongroup_apost[_idx] = apost;
        _ptr_array_neurongroup_apre[_idx] = apre;
