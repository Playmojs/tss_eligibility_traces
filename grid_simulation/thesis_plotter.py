...
if ('analysis_plots' in plots):
    ...
    ax1.axis('off')
    ax1.plot([origo[0], 26], [origo[1], 13], color = 'Black', marker = 'h', linewidth = 4, markersize = 3)
    ax1.plot([origo[0], 20], [origo[1], origo[1]], color = 'Black', linewidth = 3)
    ax1.plot([0, origo[0]], [0, origo[1]], color = 'Black', linestyle = 'dashed', linewidth = 3)
    ax1.add_patch(Arc([origo[0], origo[1]], 24, 24, angle = 0, theta1 = 0, theta2 = 16, linewidth = 3))

    ax1.text(15, 12, '1)', size = 20)
    ax1.text(22, 6, '2)', size = 20)
    ax1.text(1.7, 5, '3)', size = 20)

    fig2, ax2 = plt.subplots()
    ax2.imshow(autocorr, origin = 'lower')
    ax2.plot([cntr, 18 + cntr], [cntr, 6 + cntr], color = 'Black', marker = 'h', linewidth = 4, markersize = 4)
    ax2.plot([cntr, 10 + cntr], [cntr, cntr], color = 'Black', linewidth = 3)
    ax2.add_patch(Arc([cntr, cntr], 20, 20, angle = 0, theta1 = 0, theta2 = 16, linewidth = 3))
    ax2.axis('off')


    fig3, ax3 = plt.subplots()
    ax3.imshow(Tmp[0, dim0 - ring_start:ring_start:-1, ring_start:dim0 - ring_start])
    ax3.axis('off')

    hex_field2 = utils.createHexField(pxs, 3*sigma, 41, 'square', False).T

    inds = np.arange(48 * 48, dtype = int)
    fig4, ax4 = plt.subplots(nrows = 3, ncols = 2)

    shuf_hex = np.empty((2, 48, 48))
    ...
    for i, hex in enumerate([hex_field, hex_field2]):
        ...
        shuf_hex[i] = np.reshape(np.ndarray.flatten(hex)[inds], (48, 48))
        ax4[i, 1].imshow(shuf_hex[i], origin = 'lower')
        ax4[i, 1].axis('off')

    gs = ax4[2,0].get_gridspec()
    for ax in ax4[2, :]:
        ax.remove()

    ax_bar = fig4.add_subplot(gs[2, :])
    ...
    if save:
        for i, fig in enumerate([fig1, fig2, fig3, fig4]):
            fig.savefig(f"grid_simulation/Documents/Figures/analysis{i}", dpi = 500, bbox_inches = 'tight') 
