...
n_simuls = np.array([30, 30]) #len(sub_dirs) // n_groups
...
if not skip_calc:
    ...
    np.savez("grid_simulation/Results/analysis/" + simulation + "/gscores3", gscores = gauss_gscores)
    if include_orientation:
        print("Calculate orientation")
        mask = (gauss_gscores[:,:,-1]>0.5).nonzero()
        filtered_corr = corr_gauss[:,:,-1][mask]
        orientations, _, __ = utils.gridOrientation(filtered_corr, pxs, sigma)
        np.savez("grid_simulation/Results/analysis/" + simulation + "/orientations3", orientations = orientations, mask = mask)


gscores = np.load("grid_simulation/Results/analysis/" + simulation + "/gscores3.npz")['gscores']
...
if include_orientation:
    orientations = np.load("grid_simulation/Results/analysis/" + simulation + "/orientations3.npz")["orientations"]
    fix, ax = plt.subplots()
