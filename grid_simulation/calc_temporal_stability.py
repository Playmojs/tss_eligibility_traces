...
if not skip_calc:
    temp_var = np.var(multi_hists[:, :, 5:], axis = -4)
    shuff_var = np.var(shuffled_hists[:, :, 5:], axis = -4)
    
    temp_stability = np.mean(temp_var, axis = (-2, -1)) * 4
    shuff_stability = np.mean(shuff_var, axis = (-2, -1)) * 4
