...
base_path = 'grid_simulation/Results/data/GJ_model'
sub_dirs = utils.getSortedEntries(base_path, 'directory', True)

n_groups = 1
n_simuls = 25 #len(sub_dirs) // n_groups
n_times = len(times)
ng = 13
skip_calc = True
...
if not skip_calc:
    ...
    np.savez("grid_simulation/Results/GJ_mass", gscores = gauss_gscores)

gscores = np.load('grid_simulation/Results/GJ_mass.npz')['gscores']
